...
// - CUSTOM_API_KEY
// - CUSTOM_API_URL
// - CUSTOM_MODEL_NAME (支持多个模型，用逗号分隔)
export const API_CONFIG = {
  ...
  // 解析多个模型配置
  // 从环境变量解析模型列表，第一个作为默认模型
  parseModels: (modelString) => {
    if (!modelString) return [];
    return modelString.split(',').map(model => model.trim()).filter(model => model.length > 0);
  },

  // 获取默认模型
  getDefaultModel: (modelString) => {
    const models = API_CONFIG.parseModels(modelString);
